...
export function AlarmForm({ mode, alarmId }: AlarmFormProps) {
  const router = useRouter();
  const { alarms, addAlarm, getAlarm, updateAlarm, deleteAlarm } = useAlarmStore();
  ...
  const handleSave = useCallback(async () => {
    ...
    if (mode === 'create') {
      ...
      updateAlarm(alarmId, draft);
    }
    router.back();
  }, [draft, mode, alarmId, addAlarm, updateAlarm, ensurePermission, router]);
      ...
      >
        ...
        >
          ...
          <View style={styles.whenSection}>
            ...
            <DayPicker selectedDays={draft.repeatDays} onDaysChange={handleDaysChange} />
                      ...
                      {draft.holidayCalendarId ? (
                        <>
                          <Text style={styles.holidayCalendarName}>
                            {holidayCalendars.find((c) => c.id === draft.holidayCalendarId)?.name ??
                              'Select calendar'}
                          </Text>
          ...
          <View style={styles.cardContainer}>
            <Card style={styles.combinedCard}>
              <Entry
                ...
                label="Sound"
                sublabel={
                  draft.soundSettings.customRecordingUri
                    ? 'Custom recording'
                    : `${draft.soundSettings.voiceStyle === 'female' ? 'Female' : 'Male'}, ${draft.soundSettings.voicePersonality
                        .split('-')
          ...
          <View style={styles.cardContainer}>
            <Card style={styles.snoozeCard}>
              <View style={styles.snoozeHeader}>
                ...
                <TouchableOpacity
                  onPress={() => togglePreview(draft.fadeInDuration, draft.soundSettings)}
                  hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }}
                  style={styles.previewButton}
                >
                  <Ionicons name={isPreviewing ? 'stop' : 'play'} size={20} color={colors.accent} />
                </TouchableOpacity>
              </View>
              <FadeInPicker
          ...
          {mode === 'edit' && (
            <View style={styles.deleteButtonContainer}>
              <Button title="Delete alarm" variant="dangerOutline" onPress={handleDelete} />
            </View>
          )}
        </ScrollView>
        ...
        <View style={styles.saveButtonContainer}>
          <Button
            title={mode === 'create' ? 'Add alarm' : 'Save'}
